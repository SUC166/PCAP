...
def _repo():
    return st.secrets["github"]["repo"]   # e.g. "youruser/pcap-data"
...
def load_admins():
    """Returns list of admin dicts. Falls back to [] if not found."""
    content, _ = _get_file()
    if content is None:
        return []
...
def save_admins(admins_list):
    ...
    url = f"{GITHUB_API}/repos/{_repo()}/contents/{_path()}"
    body = {
        "message": f"PCAP admin update {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}",
        "content": encoded,
    }
    if sha:
        body["sha"] = sha
...
def create_admin(username: str, password: str, phone: str, created_by: str):
    """Creates a new admin. Returns (True,'') or (False, reason)."""
    admins = load_admins()
...
def bootstrap_needed():
    """True if no admins exist yet (first-ever run)."""
